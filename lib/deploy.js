...
const { cleanDirectoryExcept, copyDirectory, sendDingTalkMarkdown } = require("../utils");
...
async function deploy({
  ...
  try {
    ...
    sendDingTalkMarkdown({webhookUrl, atMobiles, succeed: true, projectName, env, username, atMobiles})
  } catch (e) {
    console.log("部署失败: ", e);
    spinner.fail("部署失败");
    sendDingTalkMarkdown({webhookUrl, atMobiles, succeed: false, failedReason: e.toString?.(), projectName, env, username, atMobiles})
  }
  console.log("------------- end -------------");
